...
import { TSPublishConfig, DEFAULT_CONFIG } from "./schema.js";

const CONFIG_FILE = "ts-publish.config.json";

export function loadConfig(cwd = process.cwd()): TSPublishConfig {
  const configPath = path.join(cwd, CONFIG_FILE);

  if (!fs.existsSync(configPath)) {
    throw new TSPublishError(
      "TP001",
      `Missing ${CONFIG_FILE}. Run "ts-publish init" first.`
    );
  }
...
function validateConfig(raw: any): TSPublishConfig {
  ...
  if (
    ...
    throw new TSPublishError(
      "TP006",
      "`formats` must be an array containing 'esm' and/or 'cjs'."
    );
  }
